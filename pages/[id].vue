...
const players: Ref<[Player, Player]> = ref([
  {
    ...
    name: "Andru",
    avatar:
      "https://th.bing.com/th/id/OIP.q5bNGznfObpA87RN-q2rbQHaIu?rs=1&pid=ImgDetMain",
    color: "#ACE1AF",
    tile: "x",
    win: 0,
  },
...
const startEnemyTurn = () => {
  currentPlaying.value =
    currentPlaying.value === players.value[0]
      ? players.value[1]
      : players.value[0];
};
const resetGame = () => {
...
<template>
  <div
    id="gameWrapper"
    class="mt-24 flex flex-col justify-center"
    v-if="!pending"
  >
    <StatusGameStatus
      class="mx-auto mb-8"
    ...
    <div class="grid grid-cols-3">
      <section id="player" class="flex items-center justify-center">
        <PlayerCard
          :profile="data.room.players[0]"
          :isPlaying="players[0].id === currentPlaying.id"
        />
      </section>
      <section id="board">
        <BoardGame
      ...
      <section id="enemy" class="flex items-center justify-center">
        <PlayerCard
          :profile="players[1]"
          :isPlaying="players[1].id === currentPlaying.id"
        />
      </section>
    </div>
    <dialog
      id="modalWin"
      class="modal z-10 mx-auto max-w-lg"
      :open="!!winnerPlayer?.id"
    >
      <div class="modal-box w-11/12 max-w-5xl">
        <h3 class="text-center text-2xl font-bold text-emerald-500">
        ...
        <div class="modal-action">
          <form method="dialog" class="w-full">
            <button
              type="button"
              class="w-full rounded bg-rose-500 p-2"
              @click="resetGame"
              autofocus
            >
              Play Again
    ...
    <dialog id="modal" class="modal z-10 mx-auto max-w-lg" :open="isBoardFull">
      <div class="modal-box w-11/12 max-w-5xl">
        <h3 class="text-center text-2xl font-bold text-emerald-500">
          No one's win 🤣
        </h3>
    ...
    <dialog id="start" class="modal z-10 mx-auto max-w-lg" :open="!start">
      <div class="modal-box w-11/12 max-w-5xl">
        <h3 class="text-center text-2xl font-bold">Are You Ready ?</h3>
        <div class="modal-action">
          <form method="dialog" class="w-full">
            <button
              type="button"
              class="w-full rounded bg-emerald-600 p-2 hover:cursor-pointer"
              @click="startGame"
              autofocus
            >
              Start The Game
